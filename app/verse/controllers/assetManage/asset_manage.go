...
func (m *AssetManagement) Insert(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	// ==== Insert chaincode =====
	if err = db.PutInterface(stub, key, &overall); err != nil {
		return resp.ErrorDB(err.Error())
	}

	//create IDX_uers_status_2_user_email
	err = db.CreateKeyWithNamespace(stub, KEY_ASSETDATA_TAG, KEY_ORGANIZATION, overall.AssetID, overall.OwnerID)
	if err != nil {
		return resp.ErrorDB(err.Error())
	}
...
func (m *AssetManagement) Change(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if err = db.PutInterface(stub, createKeyByID(update.AssetID), &update); err != nil {
		return resp.ErrorDB(err.Error())
	}
